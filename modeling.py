...
# ---------------------------------------------------------
# HLRRM Architecture (w/ Positional Embeddings for CausalLM)
# Hierarchical Long-Range Recurrent Memory (HLRRM)
...
class HLRRMInner(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.H_module = HLRRMBlock(config["d_model"], config["n_heads"], config["d_ff"], config["dropout"])
        self.L_module = HLRRMBlock(config["d_model"], config["n_heads"], config["d_ff"], config["dropout"])
...
class HLRRMText1(nn.Module):
    def __init__(self, config):
        ...
        self.inner_model = HLRRMInner(config)
